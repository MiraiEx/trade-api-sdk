...
class MiraiEx {
    constructor (options) {
        ...
        const allowedBaseUrl = 'https://api.miraiex.com/'
        if (options.baseUrl) {
            if (!this.isValidUrl(options.baseUrl, allowedBaseUrl)) {
                throw new Error("Invalid baseUrl provided")
            }
            this.baseUrl = options.baseUrl
        }
        else {
            this.baseUrl = allowedBaseUrl
        }
    ...
    isValidUrl(url, allowedBaseUrl) {
        try {
            const urlObj = new URL(url)
            const baseUrlObj = new URL(allowedBaseUrl)
    ...
    _internalUpdateMarkets() {
        ...
        if (!this.isValidUrl(url, this.baseUrl)) {
    ...
    getOrderBook(market) {
        return new Promise((resolve, reject) => {
            ...
            const headers = {}
            const url = `${this.baseUrl}/markets/${market}/depth`
            if (!this.isValidUrl(url, this.baseUrl)) {
                return reject({ "message": "Invalid URL" })
            }
    ...
    getLatestMarketTrades(market) {
        return new Promise((resolve, reject) => {
            ...
            const url = `${this.baseUrl}/markets/${market}/history`
            if (!this.isValidUrl(url, this.baseUrl)) {
                return reject({ "message": "Invalid URL" })
            }
    ...
    cancelAllOrdersInMarket(market) {
        return new Promise((resolve, reject) => {
            ...
            const headers = { "Content-Type": "application/json" }
            const url = `${this.baseUrl}/orders/${market}`
            if (!this.isValidUrl(url, this.baseUrl)) {
                return reject({ "message": "Invalid URL" })
            }
